...
import 'package:trinistocks_flutter/widgets/dividend_linechart.dart';
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          FutureBuilder<Map>(
            //make the API call
            future: DividendAPI.fetchDividendData(selectedSymbol, dateRange),
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                return new DividendLineChart(snapshot.data!);
              } //while the data is loading, return a progress indicator
              else
                return new LoadingWidget(loadingText: 'Loading dividend data.');
            },
          ),
        ],
      ),
