...
class PortfolioSectorBookValueHorizontalBarChart extends StatelessWidget {
  final List inputData;
  final bool? animate;

  PortfolioSectorBookValueHorizontalBarChart(this.inputData, {this.animate});

  @override
  Widget build(BuildContext context) {
    // For horizontal bar charts, set the [vertical] flag to false.
    return new SfCartesianChart(
      title: ChartTitle(text: "Sectors By Book Cost"),
  ...
  List<BarSeries<MarketValueData, String>> _createSeriesFromData() {
    ...
    for (int i = 0; i < inputData.length; i++) {
      final marketvalueData = new MarketValueData(
          inputData[i]['sector'], inputData[i]['book_cost']);
